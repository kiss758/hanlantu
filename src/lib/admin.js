...
layui.define("view", function (exports) {
  var $ = layui.jquery,
    ...
    admin = {
      ...
      layer: function (res) {
        layer.msg(`${res}`, {offset: "15px",icon: 1,time: 2000,});
      },
      adjustment: function () {
        var tr_length = $(".layui-table-main>.layui-table>tbody>tr").length; //表格内容所有的行数

        for (var i = 0; i < parseInt(tr_length); i++) {
          var t_body_height = $(".layui-table-body>.layui-table>tbody>tr")
            .eq(i)
            .height(); //表格内容高度
          var t_body_fiexd_height = $(
            ".layui-table-fixed>.layui-table-body>.layui-table>tbody>tr"
          )
            .eq(i)
            .height(); //表格固定栏内容行高度

          if (t_body_height > t_body_fiexd_height) {
            var body_height = t_body_height;
          } else {
            var body_height = t_body_fiexd_height;
          }

          $(".layui-table-main>.layui-table>tbody>tr")
            .eq(i)
            .height(body_height); //身体行高度
          $(".layui-table-fixed-l>.layui-table-body>table>tbody>tr")
            .eq(i)
            .height(body_height); //左边固定栏高度
          $(".layui-table-fixed-r>.layui-table-body>table>tbody>tr")
            .eq(i)
            .height(body_height); //右边固定栏高度
        }

        var t_thead_height = $(
          ".layui-table-box>.layui-table-header>table>thead>tr"
        )
          .eq(0)
          .height(); //表头高度
        $(".layui-table-fixed-l>.layui-table-header>table>thead>tr")
          .eq(0)
          .height(t_thead_height); //左边固定栏头部
        $(".layui-table-fixed-r>.layui-table-header>table>thead>tr")
          .eq(0)
          .height(t_thead_height); //右边固定栏头部


          // var  mainHeight = $(".layui-table-main").outerHeight(true);
          // $(".layui-table-main").height(mainHeight+23);
